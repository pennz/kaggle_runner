...
log_receiver:
	-pkill -f "$(CHECK_PORT)"
	-sudo firewall-cmd --add-port $(CHECK_PORT)/tcp
	-sudo firewall-cmd --add-port $(CHECK_PORT)/tcp --permanent
	(ncat -vkl --recv-only  -p $(CHECK_PORT) | $(UNBUFFERP) cat >> logs_check) & #(sleep 1; tail -f logs_check) &# it will be called as dep, so put it in background
