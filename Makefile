...
may_torch_gpu_setup:
	-[ -d /kaggle/input/apex-compiled-for-gpu-kernel/apex ] && (cp -r /kaggle/input/apex-compiled-for-gpu-kernel/apex . && pushd apex && pip install .) # overwrite the apex given by kernel
