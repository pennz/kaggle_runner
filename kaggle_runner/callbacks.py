...
def _check_grad(raw_opt):
    ...
    with torch.no_grad():
        ...
        normsg = torch.tensor([torch.norm(p.grad)
                               for p in pg0pl[:10] if p.grad is not None])
        ...
        norms1g = torch.tensor([torch.norm(p.grad)
                                for p in pg1pl[:10] if p.grad is not None])
...
class CheckGrad(LearnerCallback):
    def __init__(self, learn: Learner, skip_loss_step=False, batch_size=16):
        ...
        logger.debug(
            "Init Callback CheckGrad with skip_loss_step: " + str(self.skip_loss_step))
        self.losses = None
    ...
    def on_backward_begin(self, **kwargs: Any) -> None:
        ...
        self.losses.update(
            kwargs['last_loss'].detach().item(), self.batch_size)
