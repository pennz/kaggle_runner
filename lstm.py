...
BS {BATCH_SIZE}, NO_ID_IN_TRAIN {EXCLUDE_IDENTITY_IN_TRAIN}, EPOCHS {EPOCHS}, Y_TRAIN_BIN {Y_TRAIN_BIN}",
    ...
    def build_identity_model(self, identity_out_num):
        ...
        x = Bidirectional(
            CuDNNLSTM(int(LSTM_UNITS // 2), return_sequences=True))(x)

        hidden = concatenate(
            [GlobalMaxPooling1D()(x), GlobalAveragePooling1D()(x), ])
        hidden = add(
            [hidden, Dense(int(DENSE_HIDDEN_UNITS // 2),
    ...
    def run_identity_model(self, subgroup, features, labels, params):
        ...
        if RESTART_TRAIN_ID or not os.path.isfile(file_name):
            ...
            # TODO change logger
            logger.info(
                f"restore from the model file {file_name} -> done\n\n\n\n")
            ...
            if PRD_ONLY:
                ...
                for i in range(10):
                    ...
                    id_pred = kernel.run_identity_model(
                        ...
                        to_predict,
                        None,
                        params={  # TODO absctract and get params for running
                            "prefix": prefix,
