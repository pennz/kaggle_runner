...
case $STAGE in
pretrain)
    python -c "from kaggle_runner.datasets.bert import load_tokens, load_labels; tokens = load_tokens(\"$KINPUT\"); lbs = load_labels(); yt, yv = lbs[0], lbs[1]; print(vars(tokens[0]), yt[:10], pv[:10]);"
    ;;
